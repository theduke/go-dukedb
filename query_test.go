...
var _ = Describe("Query", func() {
	...
	It("Should set model", func() {
		q := Q("test_model")
		Expect(q.Model).To(Equal("test_model"))
	})

	It("Should set limit", func() {
		q := Q("test_model").Limit(22)
		Expect(q.LimitNum).To(Equal(22))
	})

	It("Should set fields", func() {
		q := Q("test_model").Fields("field1", "field2", "field3")
		Expect(q.FieldSpec).To(Equal([]string{"field1", "field2", "field3"}))
	})
	...
	It("Should FilterQ", func() {
		...
		Expect(q.Filters).To(Equal([]Filter{
			Eq("field1", "val"),
			Neq("field2", 22),
			Eq("field3", 33)}))
	})
	...
	It("Should fail OrQ with multiple filters present", func() {
		q := Q("test_model").And("field1", "val").And("field2", 22).OrQ(Eq("field1", "val"))
		Expect(q.Filters).To(Equal([]Filter{
			Eq("field1", "val"),
			Eq("field2", 22)}))
		Expect(q.Error).To(MatchError("invalid_or_with_multiple_clauses"))
	})
	...
	It("Should Not()", func() {
		...
		Expect(q.Filters).To(Equal([]Filter{Neq("field1", "val")}))
	})
})
